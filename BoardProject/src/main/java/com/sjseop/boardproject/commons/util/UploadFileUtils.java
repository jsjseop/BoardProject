...
public class UploadFileUtils {
	...
	public static String uploadFile(MultipartFile file, HttpServletRequest request) throws Exception {
		...
		byte[] fileByte = file.getBytes(); //파일 데이터
		
		//파일명 중복 방지 처리
		String uuidFileName = getUuidFileName(originalFileName);
		
		//파일 업로드 경로
		String rootPath = getRootPath(originalFileName, request); //기본경로(이미지 or 일반파일)
		String datePath = getDatePath(rootPath); //날짜 폴더 생성
		logger.info("rootPath : " + rootPath);
		logger.info("datePath : " + datePath);
		
		//서버에 파일 저장
		File target = new File(rootPath + datePath, uuidFileName); //파일 객체 생성
		FileCopyUtils.copy(fileByte, target); //파일 객체에 파일 데이터 복사
